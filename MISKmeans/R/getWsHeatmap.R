...
getWsHeatmap <- function(astudy, aCs, ws = NULL, Rowv = NA, geneOrder = NULL, ...) {
    ...
    finalRes = t(apply(resHeatmap, 1, function(x) rGN(x, aCs)))
    ...
    # Colv=NA,keysize=1.3,...)  )
    return(heatmap(finalRes, Rowv = Rowv, ColSideColors = coll, keep.dendro = TRUE, col = rgb(c(rep(0, B), (0:B)/B), 
        c((B:0)/16, rep(0, B)), rep(0, 2 * B + 1)), margins = c(1, 1), scale = "none", Colv = NA, ...))
}
