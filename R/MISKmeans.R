...
MISKmeans <- function(d, K = NULL, gamma = NULL, lambda = 0.5, alpha = 0.5, group = NULL, nstart = 20, 
	...
	# mskm1 <- MetaSpaKmeans(d, K = K, wbounds = 25, wsPre = wsPre, sampleSizeAdjust = sampleSizeAdjust)
    # mskm1 <- MetaSparseKmeans::MetaSparseKmeans(d, K = K, wbounds = 25, wsPre = wsPre, sampleSizeAdjust = sampleSizeAdjust)
    # Map(adjustedRandIndex, mskm$Cs, label)
	# Map(adjustedRandIndex, mskm1$Cs, label)
	# Map(adjustedRandIndex, res$Cs, label)
    # sum(mskm$ws != 0)
    wsPre <- mskm$ws
    Cs <- mskm$Cs
