...
study2_Expr <- BRCA_Expr2[,index_sub2] ## study 2, gene expression
study2_CNV <- BRCA_CNV2[,index_sub2] ## study 2, CNV
study2_Meth <- BRCA_Meth2[,index_sub2] ## study 2, methylation
```
...
```{r}
res_combineEC <- MISKmeans(data_combineEC,K=3,group=groups,gamma=0.7)
table(res_combineEC$ws != 0)
res_combineEC$Cs

ws_gene <- res_combineEC$ws[omicsTyps=="gene"]
table(ws_gene!=0)
ws_CNV <- res_combineEC$ws[omicsTyps=="CNV"]
table(ws_CNV!=0)
...
```{r}
getWsHeatmap(study1_Expr, res_combineEC$Cs[[1]], ws = ws_gene, Rowv=TRUE, main="S1, Expr")
getWsHeatmap(study1_CNV, res_combineEC$Cs[[1]], ws = ws_CNV, Rowv=TRUE, main="S1, CNV")

getWsHeatmap(study2_Expr, res_combineEC$Cs[[2]], ws = ws_gene, Rowv=TRUE, main="S2, Expr")
getWsHeatmap(study2_CNV, res_combineEC$Cs[[2]], ws = ws_CNV, Rowv=TRUE, main="S2, CNV")
```
